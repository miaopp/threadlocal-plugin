...
import com.qunar.flight.java.ast.parser.FormalParametersListTreeImpl;
...
public class MethodTreeImpl extends JavaTree implements MethodTree {
    ...
    public MethodTreeImpl(FormalParametersListTreeImpl parameters, @Nullable SyntaxToken defaultToken,
    ...
    public MethodTreeImpl(@Nullable TypeTree returnType, IdentifierTree simpleName,
            FormalParametersListTreeImpl parameters, @Nullable SyntaxToken throwsToken,
            ListTree<TypeTree> throwsClauses, @Nullable BlockTree block, @Nullable SyntaxToken semicolonToken) {
     ...
     *
     * @return true if overriden, null if it cannot be decided (method symbol not resolved or lack of bytecode for super
     *         types).
     */
    @CheckForNull
    ...
    private static boolean isJavaLangOverride(MemberSelectExpressionTree annotationType) {
        ...
        if (isOverride(mse.identifier()) && mse.expression().is(Kind.MEMBER_SELECT)) {
            ...
            return "lang".equals(mse.identifier().name()) && mse.expression().is(Kind.IDENTIFIER)
                    && "java".equals(((IdentifierTree) mse.expression()).name());
        }
        return false;
    }
    ...
    private boolean isParameterStringArray() {
        ...
        if (variableTree.type().is(Tree.Kind.ARRAY_TYPE)) {
            ...
            result = arrayTypeTree.type().symbolType().isClass()
