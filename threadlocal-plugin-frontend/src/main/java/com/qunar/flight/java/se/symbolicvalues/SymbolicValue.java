...
import com.qunar.flight.java.se.ExplodedGraphWalker;
...
import com.qunar.flight.java.se.constraint.TypedConstraint;
...
public class SymbolicValue {
    ...
    public static final List<SymbolicValue> PROTECTED_SYMBOLIC_VALUES = ImmutableList.of(NULL_LITERAL, TRUE_LITERAL,
            FALSE_LITERAL);
    ...
    public static boolean isDisposable(SymbolicValue symbolicValue) {
        ...
        return !PROTECTED_SYMBOLIC_VALUES.contains(symbolicValue)
                && !(symbolicValue instanceof RelationalSymbolicValue);
    }
        ...
        public List<ProgramState> setConstraint(ProgramState programState, BooleanConstraint booleanConstraint) {
            if (booleanConstraint.isTrue()) {
                ...
                if (ps.size() == 1 && ps.get(0).equals(programState)) {
                    // FIXME we already know that operand is NOT NULL, so we add a different constraint to distinguish
                    // program state. Typed Constraint
    ...
    public abstract static class BooleanExpressionSymbolicValue extends BinarySymbolicValue {
        ...
        protected static void addStates(List<ProgramState> states, List<ProgramState> newStates) {
            if (states.size() > ExplodedGraphWalker.MAX_NESTED_BOOLEAN_STATES
                    || newStates.size() > ExplodedGraphWalker.MAX_NESTED_BOOLEAN_STATES) {
