        ...
        if (expression.is(Kind.INT_LITERAL, Kind.LONG_LITERAL)) {
            ...
            if (value != null) {
                ...
                try {
                    ...
                    // Long.decode() may fail in case of very large long number written in hexadecimal. In such
                    // situation, we ignore the number.
                    // Note that Long.MAX_VALUE = "0x7FFF_FFFF_FFFF_FFFFL", but it is possible to write larger numbers
                    // in hexadecimal
