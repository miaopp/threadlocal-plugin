...
public class BytecodeVisitor extends ClassVisitor {
    ...
    BytecodeVisitor(BytecodeCompleter bytecodeCompleter, Symbols symbols, JavaSymbol.TypeJavaSymbol classSymbol,
    ...
    private JavaSymbol.TypeJavaSymbol getClassSymbol(JavaSymbol.TypeJavaSymbol classSymbol, String bytecodeName,
            int flags) {
    ...
    @Override
    public void visit(int version, int flags, String name, @Nullable String signature, @Nullable String superName,
            @Nullable String[] interfaces) {
        Preconditions.checkState(name.endsWith(classSymbol.name), "Name : '" + name + "' should ends with "
                + classSymbol.name);
     ...
     * be processed by {@link org.sonar.java.resolve.BytecodeCompleter}, therefore this method always throws
    ...
    /**
     * Invoked when current class classified as outer class of some inner class. Adds inner class as member.
     */
    private void defineInnerClass(String bytecodeName, int flags) {
        ...
        Preconditions.checkState(innerClass.owner == classSymbol, "Innerclass: " + innerClass.owner.getName()
                + " and classSymbol: " + classSymbol.getName() + " are not the same.");
        classSymbol.members.enter(innerClass);
    }

    /**
     * Invoked when current class classified as inner class. Owner of inner classes - is some outer class, which is
     * either already completed, and thus already has this inner class as member, either will be completed by
     * {@link org.sonar.java.resolve.BytecodeCompleter}, and thus will have this inner class as member (see
     * {@link #defineInnerClass(String, int)}).
     */
    private void defineOuterClass(String outerName, String innerName, int flags) {
        ...
        Preconditions.checkState(outerClassSymbol.completer == null
    ...
    @Override
    public FieldVisitor visitField(int flags, String name, String desc, @Nullable String signature,
            @Nullable Object value) {
        Preconditions.checkNotNull(name);
        Preconditions.checkNotNull(desc);
        if (!BytecodeCompleter.isSynthetic(flags)) {
            // Flags from asm lib are defined in Opcodes class and map to flags defined in Flags class
            final JavaSymbol.VariableJavaSymbol symbol = new JavaSymbol.VariableJavaSymbol(
                    bytecodeCompleter.filterBytecodeFlags(flags), name,
                    convertAsmType(org.objectweb.asm.Type.getType(desc)), classSymbol);
    ...
    @Override
    public MethodVisitor visitMethod(int flags, String name, String desc, @Nullable String signature,
            @Nullable String[] exceptions) {
        Preconditions.checkNotNull(name);
        Preconditions.checkNotNull(desc);
        if (!BytecodeCompleter.isSynthetic(flags)) {
            Preconditions.checkState((flags & Opcodes.ACC_BRIDGE) == 0,
            ...
            JavaType.MethodJavaType type = new JavaType.MethodJavaType(
                    ...
                    getCompletedClassSymbolsType(exceptions), classSymbol);
            final JavaSymbol.MethodJavaSymbol methodSymbol = new JavaSymbol.MethodJavaSymbol(
    ...
    /**
     * If at this point there is no owner of current class, then this is a top-level class, because outer classes always
     * will be completed before inner classes - see {@link #defineOuterClass(String, String, int)}. Owner of top-level
     * classes - is a package.
     */
    @Override
    ...
    private class ReadType extends SignatureVisitor {
        ...
        public SignatureVisitor visitTypeArgument(final char wildcard) {
            return new ReadType(methodSymbol) {
                ...
                public void visitEnd() {
                    ...
                    switch (wildcard) {
                    case EXTENDS:
                        typeArgument = parametrizedTypeCache.getWildcardType(this.typeRead,
                                JavaType.WildCardType.BoundType.EXTENDS);
                        break;
                    case SUPER:
                        typeArgument = parametrizedTypeCache.getWildcardType(this.typeRead,
                                JavaType.WildCardType.BoundType.SUPER);
                        break;
                    case INSTANCEOF:
                    default:
        ...
        public void visitTypeVariable(String name) {
            ...
            Preconditions.checkState(!lookup.isEmpty(), "Could not resolve type parameter: " + name + " in class "
                    + classSymbol.getName());
        ...
        public void visitEnd() {
            if (!typeArguments.isEmpty()) {
                ...
                readSymbol.complete();
                // Mismatch between type variable and type arguments means we are lacking some pieces of bytecode to
                // resolve substitution properly.
