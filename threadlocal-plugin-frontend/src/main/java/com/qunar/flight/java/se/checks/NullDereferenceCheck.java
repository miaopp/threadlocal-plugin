...
@Rule(key = "S2259", name = "Null pointers should not be dereferenced", priority = Priority.BLOCKER, tags = { "bug",
        "cert", "cwe", "owasp-a1", "owasp-a2", "owasp-a6", "security" })
...
public class NullDereferenceCheck extends SECheck {
    ...
    private ProgramState checkMemberSelect(CheckerContext context, MemberSelectExpressionTree syntaxNode,
        ...
        if (constraint != null && constraint.isNull()) {
            ...
            context.reportIssue(syntaxNode, this,
                    "NullPointerException might be thrown as '" + SyntaxTreeNameFinder.getName(syntaxNode)
                            + "' is nullable here", secondary);
            return null;
        }
        SymbolicValue targetValue = programState.peekValue();
        constraint = programState.getConstraint(targetValue);
        if (constraint == null) {
            // We dereferenced the target value for the member select, so we can assume it is not null when not already
            // known
