...
public class TypeSubstitutionSolver {
    ...
    @CheckForNull
    TypeSubstitution getTypeSubstitution(JavaSymbol.MethodJavaSymbol method, JavaType site, List<JavaType> typeParams,
    ...
    JavaType getReturnType(@Nullable JavaType returnType, JavaType site, boolean parametrizedMethodCall,
    ...
    List<JavaType> applySiteSubstitutionToFormalParameters(List<JavaType> formals, JavaType site) {
        if (isParametrizedType(site)) {
            return applySubstitutionToFormalParameters(formals,
                    ((JavaType.ParametrizedTypeJavaType) site).typeSubstitution);
        }
        return formals;
    }
    ...
    private JavaType substituteInArrayType(JavaType.ArrayJavaType arrayType, TypeSubstitution substitution) {
        ...
        if (substitutedType != null) {
            // FIXME SONARJAVA-1574 a new array type should not be created but reused if already existing for the
    ...
    TypeSubstitution getSubstitutionFromTypeParams(List<JavaType.TypeVariableJavaType> typeVariableTypes,
    ...
    private TypeSubstitution inferTypeSubstitution(JavaSymbol.MethodJavaSymbol method, List<JavaType> formals,
            List<JavaType> argTypes) {
        TypeSubstitution substitution = new TypeSubstitution();
    ...
    private TypeSubstitution inferTypeSubstitution(JavaSymbol.MethodJavaSymbol method,
            TypeSubstitution currentSubstitution, boolean isLastParam, JavaType formalType, JavaType argType) {
        if (formalType.isTagged(JavaType.TYPEVAR)) {
            ...
            if (argType.isArray()) {
                completeSubstitution(currentSubstitution, formalElementType,
                        ((JavaType.ArrayJavaType) argType).elementType);
            ...
            List<JavaType> formalTypeSubstitutedTypes = ((JavaType.ParametrizedTypeJavaType) formalType).typeSubstitution
                    .substitutedTypes();
            if (isParametrizedType(argType)) {
                List<JavaType> argTypeSubstitutedTypes = ((JavaType.ParametrizedTypeJavaType) argType).typeSubstitution
                        .substitutedTypes();
                TypeSubstitution newSubstitution = inferTypeSubstitution(method, formalTypeSubstitutedTypes,
                        argTypeSubstitutedTypes);
    ...
    private static TypeSubstitution mergeTypeSubstitutions(TypeSubstitution currentSubstitution,
            TypeSubstitution newSubstitution) {
        TypeSubstitution result = new TypeSubstitution();
        for (Map.Entry<JavaType.TypeVariableJavaType, JavaType> substitution : currentSubstitution
                .substitutionEntries()) {
    ...
    private boolean isValidSubstitution(TypeSubstitution candidate, JavaType.TypeVariableJavaType typeVar,
            JavaType typeParam, JavaType site) {
        for (JavaType bound : typeVar.bounds) {
            ...
            while (currentBound.isTagged(JavaType.TYPEVAR)) {
                ...
                if (newBound == null && isParametrizedType(site)) {
                    newBound = ((JavaType.ParametrizedTypeJavaType) site).typeSubstitution
                            .substitutedType(currentBound);
                }
                if (newBound == null) {
                    return false;
                }
