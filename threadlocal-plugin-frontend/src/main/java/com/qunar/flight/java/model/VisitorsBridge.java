...
import com.qunar.flight.java.CharsetAwareVisitor;
import com.qunar.flight.java.JavaVersionAwareVisitor;
import com.qunar.flight.java.SonarComponents;
import com.qunar.flight.java.ast.visitors.SonarSymbolTableVisitor;
import com.qunar.flight.java.resolve.SemanticModel;
import com.qunar.flight.java.se.SymbolicExecutionVisitor;
...
public class VisitorsBridge {
    ...
    public void visitFile(@Nullable Tree parsedTree) {
        semanticModel = null;
        CompilationUnitTree tree = new JavaTree.CompilationUnitTreeImpl(null, Lists.<ImportClauseTree> newArrayList(),
                Lists.<Tree> newArrayList(), null);
        ...
        JavaFileScannerContext javaFileScannerContext = createScannerContext(tree, semanticModel, analyseAccessors,
                sonarComponents, fileParsed);
        // Symbolic execution checks
        if (symbolicExecutionEnabled
                && isNotJavaLangOrSerializable(PackageUtils.packageName(tree.packageDeclaration(), "/"))) {
    ...
    private static List<JavaFileScanner> executableScanners(List<JavaFileScanner> scanners, JavaVersion javaVersion) {
        ...
        for (JavaFileScanner scanner : scanners) {
            if (!(scanner instanceof JavaVersionAwareVisitor)
                    || ((JavaVersionAwareVisitor) scanner).isCompatibleWithJavaVersion(javaVersion)) {
                results.add(scanner);
            }
        }
    ...
    protected JavaFileScannerContext createScannerContext(CompilationUnitTree tree, SemanticModel semanticModel,
            boolean analyseAccessors, SonarComponents sonarComponents, boolean fileParsed) {
        return new DefaultJavaFileScannerContext(tree, currentFile, semanticModel, analyseAccessors, sonarComponents,
                javaVersion, fileParsed);
    }
    ...
    private void createSonarSymbolTable(CompilationUnitTree tree) {
        if (sonarComponents != null) {
            SonarSymbolTableVisitor symVisitor = new SonarSymbolTableVisitor(
                    sonarComponents.symbolizableFor(currentFile), semanticModel);
