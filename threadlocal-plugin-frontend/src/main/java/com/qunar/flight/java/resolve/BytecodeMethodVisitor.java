...
import com.qunar.flight.java.resolve.JavaSymbol.MethodJavaSymbol;
...
public class BytecodeMethodVisitor extends MethodVisitor {
    ...
    /**
     * This counter counts the number of argument annotated with java.lang.Synthetic. This annotation is added by the
     * asm library for arguments that are in the method descriptor but not in the method signature. As we rely on method
     * signature (when available) we might end up with less parameter in the method symbol than in the method descriptor
     * (which makes sense as we want to be more compliant with source than with bytecode). So we assume all synthetic
     * params are the first parameters and count them as we visit. Then we substract that number from the parameter
     * index to be compliant with what is in MethodSymbol.
     */
    ...
    public AnnotationVisitor visitParameterAnnotation(int parameter, String desc, boolean visible) {
        ...
        if (annotationType.is("java.lang.Synthetic")) {
            ...
            AnnotationInstanceResolve annotationInstance = new AnnotationInstanceResolve(annotationType.getSymbol());
            methodSymbol.getParameters().scopeSymbols().get(parameter - syntheticArgs).metadata()
