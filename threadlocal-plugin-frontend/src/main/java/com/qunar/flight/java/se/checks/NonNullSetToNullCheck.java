...
import com.qunar.flight.java.resolve.Scope;
...
@Rule(key = "S2637", name = "\"@NonNull\" values should not be set to null", priority = Priority.CRITICAL, tags = {
        "bug", "misra" })
...
public class NonNullSetToNullCheck extends SECheck {
    ...
    private void checkVariable(CheckerContext context, MethodTree tree, final Symbol symbol) {
        ...
        if (nonNullAnnotation != null && isUndefinedOrNull(context, symbol)) {
            context.reportIssue(tree, this, MessageFormat.format(
                    "\"{0}\" is marked \"{1}\" but is not initialized in this constructor.", symbol.name(),
                    nonNullAnnotation));
        }
    }
    ...
    private class PreStatementVisitor extends AbstractStatementVisitor {
        ...
        public void visitNewClass(NewClassTree syntaxTree) {
            ...
            if (symbol.isMethodSymbol()) {
                ...
                checkNullArguments(syntaxTree, methodSymbol.getParameters(), argumentValues,
                        "Parameter {0} to this constructor is marked \"{1}\" but null is passed.");
            }
        }
        ...
        public void visitMethodInvocation(MethodInvocationTree syntaxTree) {
            ...
            if (symbol.isMethodSymbol()) {
                ...
                JavaSymbol.MethodJavaSymbol methodSymbol = (JavaSymbol.MethodJavaSymbol) symbol;
                checkNullArguments(syntaxTree, methodSymbol.getParameters(), argumentValues,
                        "Parameter {0} to this call is marked \"{1}\" but null is passed.");
            }
        }

        protected void checkNullArguments(Tree syntaxTree, Scope parameters, List<SymbolicValue> argumentValues,
                String message) {
            if (parameters != null) {
                ...
                if (scopeSymbols.size() < parametersToTest) {
                    // The last parameter is a variable length argument: the non-null condition does not apply to its
                    // values
        ...
        protected void checkNullArgument(Tree syntaxTree, String message, List<JavaSymbol> scopeSymbols,
                List<SymbolicValue> argumentValues, int i) {
