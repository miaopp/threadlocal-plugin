...
public class Convert {
    ...
    public static String innerClassName(String enclosingClassName, String shortName) {
        Preconditions.checkArgument(!enclosingClassName.isEmpty(), "Enclosing class name should not be empty : "
                + shortName);
        int indexEnclosing = shortName.indexOf(enclosingClassName);
        Preconditions.checkState(indexEnclosing > -1, "Error short name does not include outerclass name : "
                + shortName + " -- " + enclosingClassName);
