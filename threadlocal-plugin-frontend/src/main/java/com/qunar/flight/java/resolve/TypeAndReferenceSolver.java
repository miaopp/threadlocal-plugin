...
import com.qunar.flight.plugins.java.api.tree.ArrayTypeTree;
...
import com.qunar.flight.plugins.java.api.tree.MethodReferenceTree;
...
import com.qunar.flight.plugins.java.api.tree.UnionTypeTree;
import com.qunar.flight.plugins.java.api.tree.VariableTree;
import com.qunar.flight.plugins.java.api.tree.WildcardTree;
...
public class TypeAndReferenceSolver extends BaseTreeVisitor {
    ...
    private static void completeMetadata(JavaSymbol symbol, List<AnnotationTree> annotations) {
        for (AnnotationTree tree : annotations) {
            AnnotationInstanceResolve annotationInstance = new AnnotationInstanceResolve(
                    (JavaSymbol.TypeJavaSymbol) tree.symbolType().symbol());
            ...
            if (arguments.size() > 1 || (!arguments.isEmpty() && arguments.get(0).is(Tree.Kind.ASSIGNMENT))) {
                for (ExpressionTree expressionTree : arguments) {
                    ...
                    // TODO: Store more precise value than the expression (real value in case of literal, symbol for
                    // enums, array of values, solve constants?)
                    annotationInstance.addValue(new AnnotationValueResolve(((IdentifierTree) aet.variable()).name(),
                            aet.expression()));
                }
            } else {
                // Constant
    ...
    private Resolve.Resolution resolveMethodSymbol(Tree methodSelect, Resolve.Env methodEnv, List<JavaType> argTypes,
        ...
        if (methodSelect.is(Tree.Kind.MEMBER_SELECT)) {
            ...
            throw new IllegalStateException("Method select in method invocation is not of the expected type "
                    + methodSelect);
        }
    ...
    private JavaSymbol resolveClassType(Tree tree, Resolve.Env resolveEnv, MemberSelectExpressionTree mse) {
        ...
        JavaType parametrizedClassType = parametrizedTypeCache.getParametrizedTypeType(symbols.classType.symbol,
                typeSubstitution);
        registerType(tree, parametrizedClassType);
        return parametrizedClassType.symbol;
    }

    private Resolve.Resolution getSymbolOfMemberSelectExpression(MemberSelectExpressionTree mse, int kind,
            Resolve.Env resolveEnv) {
        ...
        if (site.kind == JavaSymbol.VAR) {
            return resolve.findIdentInType(resolveEnv, getType(mse.expression()).symbol, mse.identifier().name(),
                    JavaSymbol.VAR);
        }
    ...
    public void visitWildcard(WildcardTree tree) {
        if (tree.is(Tree.Kind.UNBOUNDED_WILDCARD)) {
            ...
            JavaType.WildCardType.BoundType boundType = tree.is(Tree.Kind.SUPER_WILDCARD) ? JavaType.WildCardType.BoundType.SUPER
                    : JavaType.WildCardType.BoundType.EXTENDS;
    ...
    public void visitBinaryExpression(BinaryExpressionTree tree) {
        ...
        JavaSymbol symbol = resolve.findMethod(semanticModel.getEnv(tree), symbols.predefClass.type,
                tree.operatorToken().text(), ImmutableList.of(left, right)).symbol();
    ...
    public void visitNewClass(NewClassTree tree) {
        ...
        resolveConstructorSymbol(newClassTreeImpl.getConstructorIdentifier(), newClassEnv,
                getParameterTypes(tree.arguments()));
    ...
    private JavaSymbol resolveConstructorSymbol(IdentifierTree identifier, Resolve.Env methodEnv,
            List<JavaType> argTypes) {
        JavaSymbol symbol = resolve.findMethod(methodEnv, (JavaType) identifier.symbolType(), "<init>", argTypes)
                .symbol();
    ...
    /**
     * Computes type of an assignment expression. Which is always a type of lvalue. For example in case of
     * {@code double d; int i; res = d = i;} type of assignment expression {@code d = i} is double.
     */
    @Override
    ...
    public void visitArrayType(ArrayTypeTree tree) {
        // FIXME(benzonico) respect invariant to set type. Required for cases like : int i[],j[]; Compute array element
        // type only if not previously computed.
    ...
    public void visitEnumConstant(EnumConstantTree tree) {
        ...
        resolveConstructorSymbol(tree.simpleName(), semanticModel.getEnv(tree),
                getParameterTypes(newClassTree.arguments()));
    }
    ...
    public void visitAnnotation(AnnotationTree tree) {
        ...
        if (arguments.size() > 1 || (!arguments.isEmpty() && arguments.get(0).is(Tree.Kind.ASSIGNMENT))) {
            ...
            for (ExpressionTree expressionTree : arguments) {
                ...
                JavaSymbol identInType = resolve.findMethod(semanticModel.getEnv(tree), getType(tree.annotationType()),
                        variable.name(), ImmutableList.<JavaType> of()).symbol();
