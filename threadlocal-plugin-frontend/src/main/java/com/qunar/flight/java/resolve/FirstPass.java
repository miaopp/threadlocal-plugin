...
import com.qunar.flight.plugins.java.api.tree.CatchTree;
...
import com.qunar.flight.plugins.java.api.tree.ForEachStatement;
...
import com.qunar.flight.plugins.java.api.tree.TryStatementTree;
...
public class FirstPass extends BaseTreeVisitor {
    ...
    /**
     * Environment. {@code env.scope.symbol} - enclosing symbol.
     */
    private Resolve.Env env;

    public FirstPass(SemanticModel semanticModel, Symbols symbols, Resolve resolve,
            ParametrizedTypeCache parametrizedTypeCache, TypeAndReferenceSolver typeAndReferenceSolver) {
    ...
    public void visitCompilationUnit(CompilationUnitTree tree) {
        ...
        if (packageDeclaration != null) {
            ...
            compilationUnitPackage = (JavaSymbol.PackageJavaSymbol) resolve.findIdentInPackage(compilationUnitPackage,
                    packageResolver.packageName, JavaSymbol.PCK);
    ...
    private class ImportResolverVisitor extends BaseTreeVisitor {
        ...
        private void enterSymbol(JavaSymbol symbol, ImportTree tree) {
            ...
            // FIXME We add all symbols to named Imports for static methods, but only the first one will be resolved as
            // we don't handle arguments.
        ...
        public void visitIdentifier(IdentifierTree tree) {
            if (JavaPunctuator.STAR.getValue().equals(tree.name())) {
                ...
                if (currentSymbol.kind == JavaSymbol.PCK) {
                    currentSymbol = resolve.findIdentInPackage(currentSymbol, tree.name(), JavaSymbol.PCK
                            | JavaSymbol.TYP);
                    ...
                    currentSymbol = resolve.findIdentInType(env, (JavaSymbol.TypeJavaSymbol) currentSymbol,
                            tree.name(), JavaSymbol.TYP | JavaSymbol.VAR).symbol();
                } else {
    ...
    public void visitClass(ClassTree tree) {
        ...
        for (TypeParameterTree typeParameterTree : tree.typeParameters()) {
            JavaSymbol.TypeVariableJavaSymbol typeVariableSymbol = new JavaSymbol.TypeVariableJavaSymbol(
                    typeParameterTree.identifier().name(), symbol);
        ...
        if (tree.is(Tree.Kind.CLASS) && classEnv.scope.lookup(CONSTRUCTOR_NAME).isEmpty()) {
            JavaSymbol.MethodJavaSymbol defaultConstructor = new JavaSymbol.MethodJavaSymbol(symbol.flags
                    & Flags.ACCESS_FLAGS, CONSTRUCTOR_NAME, symbol);
            JavaType.MethodJavaType defaultConstructorType = new JavaType.MethodJavaType(ImmutableList.<JavaType> of(),
                    null, ImmutableList.<JavaType> of(), symbol);
    ...
    public void visitMethod(MethodTree tree) {
        ...
        JavaSymbol.MethodJavaSymbol symbol = new JavaSymbol.MethodJavaSymbol(computeFlags(tree.modifiers(), tree),
                name, env.scope.owner);
        symbol.declaration = tree;
    ...
    public void visitEnumConstant(EnumConstantTree tree) {
        // JLS-8.9.3
        declareVariable(Flags.PUBLIC | Flags.STATIC | Flags.FINAL | Flags.ENUM, tree.simpleName(),
                (VariableTreeImpl) tree);
    ...
    private void declareVariable(int flags, IdentifierTree identifierTree, VariableTreeImpl tree) {
        JavaSymbol.VariableJavaSymbol symbol = new JavaSymbol.VariableJavaSymbol(flags, identifierTree.name(),
                env.scope.owner);
        symbol.declaration = tree;
        enterSymbol(tree, symbol);
        symbol.completer = completer;
