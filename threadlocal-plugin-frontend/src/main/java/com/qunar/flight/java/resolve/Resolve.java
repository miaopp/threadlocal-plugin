 ...
 * <p/>
 * Naming conventions: env - is the environment where the symbol was mentioned site - is the type of which symbol is a
 * member name - is the symbol's name
 * <p/>
...
public class Resolve {
    ...
    private Resolution findField(Env env, JavaSymbol.TypeJavaSymbol site, String name, JavaSymbol.TypeJavaSymbol c) {
        ...
        if (c.getSuperclass() != null) {
            ...
            if (resolution.symbol.kind < bestSoFar.symbol.kind) {
                resolution.type = typeSubstitutionSolver.applySiteSubstitution(resolution.symbol.type,
                        c.getSuperclass());
    ...
    private JavaSymbol findMemberType(Env env, JavaSymbol.TypeJavaSymbol site, String name, JavaSymbol.TypeJavaSymbol c) {
        ...
        for (JavaSymbol symbol : c.members().lookup(name)) {
            if (symbol.kind == JavaSymbol.TYP) {
                return isAccessible(env, site, symbol) ? symbol
        ...
        if (c.getInterfaces() == null) {
            // Invariant to check that interfaces are not set only when we are looking into the symbol we are currently
            // completing.
    ...
    private Resolution findMethod(Env env, JavaType callSite, JavaType site, String name, List<JavaType> argTypes,
    ...
    private Resolution findMethod(Env env, JavaType callSite, JavaType site, String name, List<JavaType> argTypes,
            List<JavaType> typeParams, boolean autoboxing) {
     ...
     */
    private Resolution selectBest(Env env, JavaType site, List<JavaType> argTypes, List<JavaType> typeParams,
        ...
        TypeSubstitution substitution = typeSubstitutionSolver.getTypeSubstitution(methodJavaSymbol, site, typeParams,
                argTypes);
     ...
     */
    private boolean isArgumentsAcceptable(List<JavaType> argTypes, List<JavaType> formals, boolean isVarArgs,
            boolean autoboxing) {
        ...
        for (int i = 1; i <= nbArgToCheck; i++) {
            ...
            if (!isAcceptableType(argType, lastFormal.elementType, autoboxing)
                    && (nbArgToCheck != 1 || !types.isSubtype(argType, lastFormal))) {
                return false;
            }
        }
    ...
    private boolean isAcceptableType(JavaType arg, JavaType formal, boolean autoboxing) {
        ...
        return types.isSubtype(arg.erasure(), formal.erasure())
                || (autoboxing && isAcceptableByAutoboxing(arg, formal.erasure()));
    }
     ...
     * <p/>
     * Symbol is accessible only if not overridden by another symbol. If overridden, then strictly speaking it is not a
     * member.
     */
    private static boolean isAccessible(Env env, JavaSymbol.TypeJavaSymbol site, JavaSymbol symbol) {
        ...
        case 0:
            return (env.packge == symbol.packge()) && isAccessible(env, site) && isInheritedIn(symbol, site)
        ...
        case Flags.PROTECTED:
            return ((env.packge == symbol.packge()) || isProtectedAccessible(symbol, env.enclosingClass, site))
                    && isAccessible(env, site) && notOverriddenIn(site, symbol);
        default:
            throw new IllegalStateException();
        }
    }
    ...
    private static boolean isProtectedAccessible(JavaSymbol symbol, JavaSymbol.TypeJavaSymbol c,
    ...
    /**
     * Resolution holds the symbol resolved and its type in this context. This is required to handle type substitution
     * for generics.
     */
