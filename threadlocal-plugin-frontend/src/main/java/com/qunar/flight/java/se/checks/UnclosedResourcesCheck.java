...
import com.qunar.flight.java.se.SymbolicValueFactory;
...
@Rule(key = "S2095", name = "Resources should be closed", priority = Priority.BLOCKER, tags = { "bug", "cert", "cwe",
        "denial-of-service", "leak", "security" })
@ActivatedByDefault
...
public class UnclosedResourcesCheck extends SECheck {
    ...
    private static final String[] IGNORED_CLOSEABLE_SUBTYPES = { "java.io.ByteArrayOutputStream",
            "java.io.ByteArrayInputStream", "java.io.CharArrayReader", "java.io.CharArrayWriter",
    ...
    private static class PreStatementVisitor extends CheckerTreeNodeVisitor {
        ...
        private SymbolicValue getTargetValue(MethodInvocationTree syntaxNode) {
            final ExpressionTree targetExpression = ((MemberSelectExpressionTree) syntaxNode.methodSelect())
                    .expression();
        ...
        private void closeResultSetsRelatedTo(SymbolicValue value) {
            for (Map.Entry<SymbolicValue, ObjectConstraint> constrainedValue : programState.getValuesWithConstraints(
                    Status.OPENED).entrySet()) {
                if (constrainedValue.getKey() instanceof ResourceWrapperSymbolicValue) {
                    ...
                    if (value.equals(rValue.dependent)) {
                        programState = programState.addConstraint(rValue,
                                constrainedValue.getValue().withStatus(Status.CLOSED));
                    }
                }
            }
        }
        ...
        public void visitNewClass(NewClassTree syntaxNode) {
            if (isOpeningResource(syntaxNode)) {
                ...
                if (!(instanceValue instanceof ResourceWrapperSymbolicValue)) {
                    programState = programState.addConstraint(instanceValue, new ObjectConstraint(false, false,
                            syntaxNode, Status.OPENED));
                }
            }
        }
        ...
        public void visitMethodInvocation(MethodInvocationTree syntaxNode) {
            if (syntaxNode.methodSelect().is(Tree.Kind.MEMBER_SELECT) && needsClosing(syntaxNode.symbolType())) {
                ...
                if (targetExpression.is(Tree.Kind.IDENTIFIER) && !isWithinTryHeader(syntaxNode)
                    ...
                    programState = programState.addConstraint(programState.peekValue(), new ObjectConstraint(false,
                            false, syntaxNode, Status.OPENED));
                }
            }
        }
