...
import com.qunar.flight.java.collections.AVLTree;
import com.qunar.flight.java.collections.PMap;
...
public class ProgramState {
    ...
    public static final ProgramState EMPTY_STATE = new ProgramState(AVLTree.<Symbol, SymbolicValue> create(),
            AVLTree.<SymbolicValue, Integer> create(), AVLTree.<SymbolicValue, Constraint> create()
            ...
            AVLTree.<ExplodedGraph.ProgramPoint, Integer> create(), Lists.<SymbolicValue> newLinkedList());
    ...
    private static PMap<SymbolicValue, Integer> decreaseReference(PMap<SymbolicValue, Integer> givenReferences,
            SymbolicValue sv) {
    ...
    private static PMap<SymbolicValue, Integer> increaseReference(PMap<SymbolicValue, Integer> givenReferences,
            SymbolicValue sv) {
        Integer value = givenReferences.get(sv);
        if (value == null) {
    ...
    private static boolean isDisposable(SymbolicValue symbolicValue, @Nullable Object constraint) {
        return SymbolicValue.isDisposable(symbolicValue)
                && (constraint == null || !(constraint instanceof ObjectConstraint) || ((ObjectConstraint) constraint)
                        .isDisposable());
    }
    ...
    public ProgramState cleanupDeadSymbols(Set<Symbol> liveVariables) {
        ...
        for (Iterator<Map.Entry<Symbol, SymbolicValue>> iter = newValues.entriesIterator(); iter.hasNext();) {
            ...
            if (isLocalVariable(symbol) && !liveVariables.contains(symbol)) {
                ...
                if (!isReachable(symbolicValue, newReferences)
                        && isDisposable(symbolicValue, newConstraints.get(symbolicValue))
        ...
        return newProgramState ? new ProgramState(newValues, newReferences, newConstraints, visitedPoints, stack)
                : this;
    }
    ...
    public ProgramState cleanupConstraints() {
        ...
        for (Iterator<Map.Entry<SymbolicValue, Constraint>> iter = newConstraints.entriesIterator(); iter.hasNext();) {
            ...
            if (!isReachable(symbolicValue, newReferences) && isDisposable(symbolicValue, next.getValue())
                    && !inStack(stack, symbolicValue)) {
                if (!newProgramState) {
                    newProgramState = true;
                }
