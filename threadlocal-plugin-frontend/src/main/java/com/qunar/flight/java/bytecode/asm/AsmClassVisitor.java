 ...
 */
package com.qunar.flight.java.bytecode.asm;
...
public class AsmClassVisitor extends ClassVisitor {
    ...
    @Override
    public void visit(int version, int accessFlags, String internalName, String signature, String superClass,
            String[] interfaces) {
        if (asmClass.getDetailLevel() == AsmClassProvider.DETAIL_LEVEL.NOTHING) {
            asmClass.setAccessFlags(accessFlags);
            if (asmClass.isInterface()) {
                if (interfaces.length == 1) {
                    asmClass.setSuperClass(asmClassProvider.getClass(interfaces[0],
                            AsmClassProvider.DETAIL_LEVEL.STRUCTURE));
                }
            } else {
                if (superClass != null) {
                    asmClass.setSuperClass(asmClassProvider.getClass(superClass,
                            AsmClassProvider.DETAIL_LEVEL.STRUCTURE));
                }
                for (String interfaceName : interfaces) {
                    asmClass.addInterface(asmClassProvider.getClass(interfaceName,
                            AsmClassProvider.DETAIL_LEVEL.STRUCTURE));
                }
            }
            if (signature != null) {
                String[] internalNames = AsmSignature.extractInternalNames(signature);
                AsmClass[] asmClasses = internalNamesToAsmClasses(internalNames, AsmClassProvider.DETAIL_LEVEL.NOTHING);
    ...
    public MethodVisitor visitMethod(int access, String methodName, String description, String signature,
        ...
        AsmClass[] asmExceptionClasses = internalNamesToAsmClasses(exceptions, AsmClassProvider.DETAIL_LEVEL.STRUCTURE);
        method.addUsesOfClasses(asmExceptionClasses);
        method.addThrowsOfClasses(asmExceptionClasses);

        if (level == AsmClassProvider.DETAIL_LEVEL.STRUCTURE_AND_CALLS) {
    ...
    private static boolean isInheritedMethodSignature(AsmClass parent, String key) {
        if (parent.getSuperClass() != null
                && (parent.getSuperClass().getMethod(key) != null || isInheritedMethodSignature(parent.getSuperClass(),
                        key))) {
            return true;
        }
