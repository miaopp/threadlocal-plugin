...
public class SonarComponents implements BatchExtension {
    ...
    private ActiveRulesBuilder activeRulesBuilder = new ActiveRulesBuilder();

    public SonarComponents(FileLinesContextFactory fileLinesContextFactory, ResourcePerspectives resourcePerspectives,
            FileSystem fs, JavaClasspath javaClasspath, JavaTestClasspath javaTestClasspath, SensorContext context,
            CheckFactory checkFactory) {
        this(fileLinesContextFactory, resourcePerspectives, fs, javaClasspath, javaTestClasspath, checkFactory,
                context, null);
    }

    public SonarComponents(FileLinesContextFactory fileLinesContextFactory, ResourcePerspectives resourcePerspectives,
            FileSystem fs, JavaClasspath javaClasspath, JavaTestClasspath javaTestClasspath, CheckFactory checkFactory,
            SensorContext context, @Nullable CheckRegistrar[] checkRegistrars) {
        RuleKey ruleKey = RuleKey.of("myrepo", "ThreadLocalRemove");
        activeRulesBuilder.create(ruleKey).activate();
        ruleKey = RuleKey.of("myrepo", "CycleBetweenPackages");
        activeRulesBuilder.create(ruleKey).activate();
        ...
        this.checkFactory = new CheckFactory(activeRulesBuilder.build());
        ...
        if (checkRegistrars != null) {
            ...
            for (CheckRegistrar checkClassesRegister : checkRegistrars) {
                ...
                registerCheckClasses(registrarContext.repositoryKey(),
                        Lists.newArrayList(checkClasses != null ? checkClasses
                                : new ArrayList<Class<? extends JavaCheck>>()));
            }
        }
    }
    ...
    public List<CodeVisitor> checkClasses() {
    ...
    void reportIssueAfterSQ52(AnalyzerMessage analyzerMessage, RuleKey key, InputPath inputPath, Double cost) {
        ...
        if (textSpan == null) {
            ...
            issue.setPrimaryLocation((InputFile) inputPath, analyzerMessage.getMessage(), textSpan.startLine,
                    textSpan.startCharacter, textSpan.endLine, textSpan.endCharacter);
        }
        for (AnalyzerMessage location : analyzerMessage.secondaryLocations) {
            ...
            issue.addSecondaryLocation(inputFromIOFile(location.getFile()), secondarySpan.startLine,
                    secondarySpan.startCharacter, secondarySpan.endLine, secondarySpan.endCharacter,
                    location.getMessage());
        }
        issue.save();
    }

    private void reportIssueBeforeSQ52(InputPath inputFile, RuleKey key, @Nullable Double cost, String message,
            @Nullable Integer line) {
        Issuable issuable = issuableFor(inputFile);
        if (issuable != null) {
            Issuable.IssueBuilder newIssueBuilder = issuable.newIssueBuilder().ruleKey(key).message(message).line(line)
                    .effortToFix(cost);
        ...
        if (cost == null) {
            ...
            if (linear != null || linearWithOffset != null) {
                throw new IllegalStateException(
                        "A check annotated with a linear sqale function should provide an effort to fix");
            }
        }
    }
