...
import com.qunar.flight.java.resolve.JavaType.ParametrizedTypeJavaType;
...
public class Types {
    ...
    public boolean isSubtype(JavaType t, JavaType s) {
        ...
        if (t == s) {
            ...
            case JavaType.CHAR:
                result = t.tag == s.tag
                        || (t.tag + /* skip char for byte and short for char */2 <= s.tag && s.tag <= JavaType.DOUBLE);
                break;
            case JavaType.SHORT:
            case JavaType.INT:
            case JavaType.LONG:
            ...
            case JavaType.ARRAY:
                if (t.tag != s.tag) {
                    // t is array, if tags are different then the only way t is subtype of s is s to be object ie:
                    // superclass of arrayClass
                    result = t.getSymbol().getSuperclass() == s;
                    break;
                }
                result = isSubtype(((JavaType.ArrayJavaType) t).elementType(),
                        ((JavaType.ArrayJavaType) s).elementType());
                break;
            case JavaType.CLASS:
    ...
    /**
     * Compute the "Least Upper Bound" ("lub", jls8 §4.10.4) of a list of type. The "lub" is a shared supertype that is
     * more specific than any other shared supertype (that is, no other shared supertype is a subtype of the least upper
     * bound)
     *
     * Parameterized types are currently ignored, as the method is used only to handle Union Types Trees, themselves
     * being used only in catch trees. Note that Exceptions (any subclass of Throwable) cannot be generic (jls8 §8.1.2,
     * §11.1.1: "compile-time error if a generic class is a direct or indirect subclass of Throwable")
     *
     * @param types
    ...
    /**
     * Let MEC, the minimal erased candidate set for U1 ... Uk, be: MEC = { V | V in EC, and for all W != V in EC, it is
     * not the case that W <: V }
     * 
