...
@Rule(key = "S2222", name = "Locks should be released", priority = Priority.CRITICAL, tags = { "bug", "cwe",
        "multi-threading" })
@ActivatedByDefault
...
public class LocksNotUnlockedCheck extends SECheck {
    ...
    private static class TryLockSymbolicValue extends SymbolicValue {
        ...
        public List<ProgramState> setConstraint(ProgramState programState, BooleanConstraint booleanConstraint) {
            if (BooleanConstraint.TRUE.equals(booleanConstraint)) {
                return ImmutableList.of(programState.addConstraint(operand, new ObjectConstraint(false, false,
                        syntaxNode, Status.LOCKED)));
            } else {
                return ImmutableList.of(programState.addConstraint(operand, new ObjectConstraint(syntaxNode,
                        Status.UNLOCKED)));
            }
        }
        ...
        private void visitMethodInvocationWithIdentifierTarget(final String methodName, final IdentifierTree target) {
            if (!isMemberSelectActingOnField(target)) {
                ...
                if (LOCK_METHOD_NAME.equals(methodName)) {
                    programState = programState.addConstraint(symbolicValue, new ObjectConstraint(false, false, target,
                            Status.LOCKED));
                    ...
                    programState = programState.addConstraint(symbolicValue, new ObjectConstraint(target,
                            Status.UNLOCKED));
                }
            }
        }
    ...
    public void checkEndOfExecutionPath(CheckerContext context, ConstraintManager constraintManager) {
        ...
        for (ObjectConstraint constraint : constraints) {
            ...
            context.reportIssue(syntaxNode, this, "Unlock \"" + syntaxNode.toString()
                    + "\" along all executions paths of this method.");
        }
    }
}
