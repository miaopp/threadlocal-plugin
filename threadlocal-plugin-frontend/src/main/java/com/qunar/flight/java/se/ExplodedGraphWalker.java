...
import com.qunar.flight.java.cfg.CFG;
import com.qunar.flight.java.cfg.LiveVariables;
import com.qunar.flight.java.matcher.MethodMatcher;
import com.qunar.flight.java.model.JavaTree;
import com.qunar.flight.java.se.checks.ConditionAlwaysTrueOrFalseCheck;
import com.qunar.flight.java.se.checks.LocksNotUnlockedCheck;
import com.qunar.flight.java.se.checks.NonNullSetToNullCheck;
import com.qunar.flight.java.se.checks.NullDereferenceCheck;
import com.qunar.flight.java.se.checks.SECheck;
import com.qunar.flight.java.se.checks.UnclosedResourcesCheck;
...
import com.qunar.flight.plugins.java.api.semantic.SymbolMetadata;
import com.qunar.flight.plugins.java.api.semantic.Type;
...
import com.qunar.flight.plugins.java.api.tree.DoWhileStatementTree;
import com.qunar.flight.plugins.java.api.tree.ExpressionTree;
import com.qunar.flight.plugins.java.api.tree.ForStatementTree;
import com.qunar.flight.plugins.java.api.tree.IdentifierTree;
import com.qunar.flight.plugins.java.api.tree.IfStatementTree;
...
import com.qunar.flight.plugins.java.api.tree.NewArrayTree;
...
import com.qunar.flight.plugins.java.api.tree.WhileStatementTree;
...
public class ExplodedGraphWalker extends BaseTreeVisitor {
    ...
    private static final MethodMatcher SYSTEM_EXIT_MATCHER = MethodMatcher.create().typeDefinition("java.lang.System")
            .name("exit").addParameter("int");
    private static final MethodMatcher OBJECT_WAIT_MATCHER = MethodMatcher.create().typeDefinition("java.lang.Object")
            .name("wait").withNoParameterConstraint();
    ...
    ExplodedGraphWalker(JavaFileScannerContext context) {
        ...
        this.checkerDispatcher = new CheckerDispatcher(this, context, Lists.<SECheck> newArrayList(
                alwaysTrueOrFalseChecker, new NullDereferenceCheck(), new UnclosedResourcesCheck(),
    ...
    private ExplodedGraphWalker(JavaFileScannerContext context,
            ConditionAlwaysTrueOrFalseCheck alwaysTrueOrFalseChecker, List<SECheck> seChecks) {
    ...
    private void execute(MethodTree tree) {
        ...
        LOG.debug("Exploring Exploded Graph for method " + tree.simpleName().name() + " at line "
        ...
        while (!workList.isEmpty()) {
            steps++;
            if (steps > MAX_STEPS) {
                throw new MaximumStepsReachedException("reached limit of " + MAX_STEPS + " steps for method "
                        + tree.simpleName().name() + " in class " + tree.symbol().owner().name());
            }
            // LIFO:
            ...
            try {
                ...
                throw new MaximumStepsReachedException("reached maximum number of " + MAX_NESTED_BOOLEAN_STATES
                        + " branched states for method " + tree.simpleName().name() + " in class "
                        + tree.symbol().owner().name(), e);
            }
        }
    ...
    private Iterable<ProgramState> startingStates(MethodTree tree, ProgramState ps) {
        ...
        for (final VariableTree variableTree : tree.parameters()) {
            ...
            if (isEqualsMethod || parameterCanBeNull(variableTree)) {
                startingStates = Iterables.concat(Iterables.transform(startingStates,
                        new Function<ProgramState, List<ProgramState>>() {
                            @Override
    ...
    /**
     * Required for accurate reporting. If condition is && or || expression, then return its right operand.
     */
    ...
    private void handleBranch(CFG.Block programPosition, Tree condition, boolean checkPath) {
        ...
        ExplodedGraph.ProgramPoint falseBlockProgramPoint = new ExplodedGraph.ProgramPoint(
                programPosition.falseBlock(), 0);
        ...
        ExplodedGraph.ProgramPoint trueBlockProgramPoint = new ExplodedGraph.ProgramPoint(programPosition.trueBlock(),
                0);
    ...
    private void executeAssignement(AssignmentExpressionTree tree) {
        ...
        if (variable.is(Tree.Kind.IDENTIFIER)) {
            ...
            SymbolicValue value = tree.is(Tree.Kind.ASSIGNMENT) ? unstack.values.get(1) : constraintManager
                    .createSymbolicValue(tree);
            programState = unstack.state;
    ...
    private void executeUnaryExpression(Tree tree) {
        ...
        if (tree.is(Tree.Kind.POSTFIX_DECREMENT, Tree.Kind.POSTFIX_INCREMENT, Tree.Kind.PREFIX_DECREMENT,
                Tree.Kind.PREFIX_INCREMENT) && ((UnaryExpressionTree) tree).expression().is(Tree.Kind.IDENTIFIER)) {
            programState = programState.put(((IdentifierTree) ((UnaryExpressionTree) tree).expression()).symbol(),
                    unarySymbolicValue);
        }
    ...
    private void logState(MethodInvocationTree mit) {
        if (mit.methodSelect().is(Tree.Kind.IDENTIFIER)
                && "printState".equals(((IdentifierTree) mit.methodSelect()).name())) {
    ...
    public void enqueue(ExplodedGraph.ProgramPoint programPoint, ProgramState programState, boolean exitPath) {
        ...
        ExplodedGraph.Node cachedNode = explodedGraph.getNode(programPoint,
                programState.visitedPoint(programPoint, nbOfExecution + 1));
    ...
    private void checkExplodedGraphTooBig(ProgramState programState) {
        ...
        if (steps + workList.size() > MAX_STEPS / 2 && programState.constraintsSize() > 75) {
            throw new ExplodedGraphTooBigException(
                    "Program state constraints are too big : stopping Symbolic Execution for method "
                            + methodTree.simpleName().name() + " in class " + methodTree.symbol().owner().name());
        }
    }

    /**
     * This class ensures that the SE checks are placed in the correct order for the ExplodedGraphWalker In addition,
     * checks that are needed for a correct ExplodedGraphWalker processing are provided in all cases.
     *
     */
