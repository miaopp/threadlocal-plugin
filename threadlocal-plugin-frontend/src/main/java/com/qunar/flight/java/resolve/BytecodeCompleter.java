...
import com.qunar.flight.java.bytecode.ClassLoaderBuilder;
...
public class BytecodeCompleter implements JavaSymbol.Completer {
    ...
    private static final int ACCEPTABLE_BYTECODE_FLAGS = Flags.ACCESS_FLAGS | Flags.INTERFACE | Flags.ANNOTATION
            | Flags.ENUM | Flags.STATIC | Flags.FINAL | Flags.SYNCHRONIZED | Flags.VOLATILE | Flags.TRANSIENT
            | Flags.VARARGS | Flags.NATIVE | Flags.ABSTRACT | Flags.STRICTFP | Flags.DEPRECATED;
    ...
    public void complete(JavaSymbol symbol) {
        ...
        if (classReader != null) {
            classReader.accept(new BytecodeVisitor(this, symbols, (JavaSymbol.TypeJavaSymbol) symbol,
                    parametrizedTypeCache), ClassReader.SKIP_CODE | ClassReader.SKIP_FRAMES | ClassReader.SKIP_DEBUG);
        }
    }
    ...
    // FIXME(Godin): or parameter must be renamed, or should not receive flat name, in a former case - first
    // transformation in this method seems useless
    JavaSymbol.TypeJavaSymbol getClassSymbol(String bytecodeName, int flags) {
        return getClassSymbol(null, bytecodeName, flags);
    }

    public JavaSymbol.TypeJavaSymbol getClassSymbol(@Nullable JavaSymbol.TypeJavaSymbol classSymbolOwner,
            String bytecodeName, int flags) {
        ...
        if (symbol == null) {
            ...
            if (owner != null) {
                // handle innerClasses
                symbol = new JavaSymbol.TypeJavaSymbol(filterBytecodeFlags(flags), Convert.innerClassName(
                        Convert.shortName(owner.getFullyQualifiedName()), shortName), owner);
            } else {
    ...
    /**
     * <b>Note:</b> Attempt to find something like "java.class" on case-insensitive file system can result in unwanted
     * loading of "JAVA.class". This method performs check of class name within file in order to avoid such situation.
     * This is definitely not the best solution in terms of performance, but acceptable for now.
     *
     * @return symbol for requested class, if corresponding class file exists, and
     *         {@link com.qunar.flight.java.resolve.Resolve.JavaSymbolNotFound} otherwise
     */
