...
public class SecondPass implements JavaSymbol.Completer {
    ...
    public SecondPass(SemanticModel semanticModel, Symbols symbols, ParametrizedTypeCache parametrizedTypeCache,
    ...
    private void complete(JavaSymbol.TypeJavaSymbol symbol) {
        ...
        if ("".equals(symbol.name)) {
            ...
            // FIXME(Godin): This case avoids NPE which occurs because semanticModel has no associations for anonymous
            // classes.
    ...
    private void populateSuperclass(JavaSymbol.TypeJavaSymbol symbol, Resolve.Env env, JavaType.ClassJavaType type) {
        ...
        if (superClassTree != null) {
            ...
            JavaType.TypeVariableJavaType enumParameter = (JavaType.TypeVariableJavaType) enumParameters.lookup("E")
                    .get(0).type();
            type.supertype = parametrizedTypeCache.getParametrizedTypeType(symbols.enumType.symbol,
                    new TypeSubstitution().add(enumParameter, type));
            ...
            // For INTERFACE JLS8 9.1.3: While every class is an extension of class Object, there is no single interface
            // of which all interfaces are
            // extensions.
    ...
    private static void checkHierarchyCycles(JavaType baseType) {
        ...
        while (type != null) {
            if (!types.add(type)) {
                throw new IllegalStateException("Cycling class hierarchy detected with symbol : "
    ...
    private void complete(JavaSymbol.MethodJavaSymbol symbol) {
        ...
        JavaType.MethodJavaType methodType = new JavaType.MethodJavaType(argTypes, returnType, thrownTypes.build(),
                (JavaSymbol.TypeJavaSymbol) symbol.owner);
    ...
    private JavaType resolveType(Resolve.Env env, Tree tree) {
        ...
        // FIXME(benzonico) as long as Variables share the same node type, (int i,j; or worse : int i[], j[];) check
        // nullity to respect invariance.
    ...
    private static boolean checkTypeOfTree(Tree tree) {
        return tree.is(Tree.Kind.MEMBER_SELECT, Tree.Kind.IDENTIFIER, Tree.Kind.PARAMETERIZED_TYPE,
                Tree.Kind.ARRAY_TYPE, Tree.Kind.UNION_TYPE, Tree.Kind.PRIMITIVE_TYPE, Tree.Kind.INFERED_TYPE);
    }
