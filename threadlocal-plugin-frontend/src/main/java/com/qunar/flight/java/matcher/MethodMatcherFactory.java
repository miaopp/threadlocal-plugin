...
public class MethodMatcherFactory {
    ...
    public static void collectArguments(String descriptor, Matcher initialMatcher, int groupOffset,
        ...
        if ("(".equals(initialMatcher.group(groupOffset))) {
            ...
            if (!")".equals(remainder)) {
                ...
                if (matchedLength < remainder.length()) {
                    throw new IllegalArgumentException("Illegal method or constructor arguments specification: "
                            + descriptor);
                }
            }
        } else {
