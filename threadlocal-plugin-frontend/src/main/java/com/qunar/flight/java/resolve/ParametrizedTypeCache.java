...
import com.qunar.flight.java.resolve.JavaType.WildCardType.BoundType;
...
public class ParametrizedTypeCache {
    ...
    private Map<JavaType, Map<JavaType.WildCardType.BoundType, JavaType.WildCardType>> wildcardCache = Maps
            .newHashMap();
    ...
    public JavaType getParametrizedTypeType(JavaSymbol.TypeJavaSymbol symbol, TypeSubstitution typeSubstitution) {
        ...
        if (typeCache.get(symbol).get(typeSubstitution) == null) {
            typeCache.get(symbol)
                    .put(typeSubstitution, new JavaType.ParametrizedTypeJavaType(symbol, typeSubstitution));
        }
